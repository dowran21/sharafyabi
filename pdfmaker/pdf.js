...
async function OrderGenerator (data){
    ...
    doc
        .fontSize(10)
        .fill("#000000").text('Код', itemCodeX+2, tableTop, {height:22, expanded:true}).highlight(itemCodeX, tableTop-4, descriptionX-itemCodeX-2, 22, {color:"#4C844C"})
    ...
    for (i = 0; i < order_items?.length; i++) {
        ...
        doc
            ...
            .text(`${item?.discount_value ? item?.discount_value : `` }`, discountX+2, y, {height:15}).highlight(discountX, y-4, priceX-discountX-2, 16, {color:`${i%2 ?`#799E79` : `#FFFFFF`}`})
